...
const state = { meta: null, fixtures: [], squad: [], appearances: [], formations: new Map(),
  motm: new Map(), rowIds: [], phaseView: null };
...
function dataFlags() {
  const { offBy, unknown, orphans, conflict, twice, notOurs } = L.dataChecks(state.fixtures, state.appearances,
    state.squad, state.meta.team, state.formations, state.motm, state.rowIds);
  if (!offBy.length && !unknown.length && !orphans.length && !twice.length && !notOurs.length) return '';
  const bits = [];
