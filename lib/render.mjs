...
function matchPanel(f) {
  ...
  if (spots) L.layout(spots).lines.forEach((l) => l.players.forEach((p) => started.add(p.player)));
  ...
  const onLater = ap.map((a) => a.player).filter((p) => !started.has(p)).sort();
  ...
  if (!L.isPlayed(f)) {
    ...
    run.push(scored.length
      ? `<div class="strip"><span class="lbl">${icon('i-ball')}Scored</span>${pills(scored, 'goals')}</div>`
      : '<div class="strip"><span class="lbl">Scored</span><span class="dim">nobody</span></div>');
    if (set.length) run.push(`<div class="strip"><span class="lbl">${icon('i-boot')}Assists</span>${pills(set, 'assists')}</div>`);
    if (onLater.length) {
      run.push(`<div class="strip"><span class="lbl">${spots ? 'Came on' : 'Played'}</span>
        ${onLater.map((p) => `<span class="sub">${face(p)}${esc(playerName(p))}</span>`).join('')}</div>`);
    }
  }
