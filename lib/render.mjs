...
const state = { meta: null, fixtures: [], squad: [], appearances: [] };
...
function tableHTML(t) {
  return `<table class="tbl"><thead><tr>
    ...
    const last5 = L.teamMatches(state.fixtures, r.team).slice(0, 5).reverse();
    return `
    <tr class="tr${r.team === state.meta.team ? ' me' : ''}" data-team="${esc(r.team)}"
    ...
    <tr class="det" hidden><td colspan="11">${teamPanel(r)}</td></tr>`;
  }).join('')}</tbody></table>`;
}
...
function teamPanel(row) {
  const ms = L.teamMatches(state.fixtures, row.team);
  const head = `<div class="dh"><b>${esc(row.team)}</b>
    <span class="dim">${ordinal(row.pos)} · ${row.pts} pt${row.pts === 1 ? '' : 's'} from ${row.p}
    · ${row.gf}–${row.ga} on the season</span></div>`;
  if (!ms.length) {
    return `<div class="tdet">${head}<p class="hint" style="margin:6px 0 0">No results yet.</p></div>`;
  }
  return `<div class="tdet">${head}<ul class="rows">${ms.map((m) => `
...
function chart() {
  const { dates, teams, series } = L.rankHistory(state.fixtures);
  if (!dates.length) {
  ...
  const lane = (t) => {
    ...
    const segs = pts.slice(1).map((p, i) => `
      <line class="seg${p.played ? '' : ' dim'}" x1="${x(i)}" y1="${y(pts[i].pos)}"
        x2="${x(i + 1)}" y2="${y(p.pos)}"/>`).join('');
    const nodes = pts.map((p, i) => `
      <g class="nd${p.played ? '' : ' dim'}">
        <circle cx="${x(i)}" cy="${y(p.pos)}" r="9"/>
        <text x="${x(i)}" y="${y(p.pos)}">${p.pos}</text>
        <title>${esc(t)} — ${esc(shortDate(p.date))}: ${ordinal(p.pos)}, ${p.pts} pts from ${p.p} played${p.played ? '' : ' (no result this week)'}</title>
      </g>`).join('');
    const last = pts[pts.length - 1];
    return `<g class="tm${mine ? ' me' : ''}">${segs}${nodes}
      <text class="nm" x="${x(n - 1) + 17}" y="${y(last.pos)}">${esc(t)}</text></g>`;
  };
  ...
  return `<div class="chartwrap">
    <svg class="bump" width="${W.toFixed(0)}" height="${H}" viewBox="0 0 ${W.toFixed(0)} ${H}" role="img"
      ...
      ${cols}${teams.filter((t) => t !== me).map(lane).join('')}${teams.includes(me) ? lane(me) : ''}
    </svg></div>
    <div class="legend">
      <span><i class="me"></i>${esc(me)}</span>
      <span><i></i>Played that week</span>
      <span><i class="off"></i>No result in yet</span>
      <span class="dim">Ranked on points, goal difference, goals for, goals against</span>
    </div>`;
}
