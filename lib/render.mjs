...
/* ---------- skin ----------
   Two looks, one page. The choice lives on <html> so CSS alone decides what
   changes, and in localStorage so it survives a reload. Storage can throw in a
   private window, so every touch of it is wrapped: a viewer who cannot store a
   preference still gets a working page, just the classic one.            */

const SKINS = [['', 'Classic'], ['bold', 'Bold'], ['terrace', 'Terrace']];
const skin = () => document.documentElement.dataset.skin || '';

function setSkin(name) {
  if (name) document.documentElement.dataset.skin = name;
  else delete document.documentElement.dataset.skin;
  try { localStorage.setItem('ctb-skin', name || 'classic'); } catch (e) { /* no storage, no memory */ }
  render();
}

/* ---------- dashboard ---------- */

// A results crawl, the way a Saturday afternoon has one. It is decoration, so
// it is hidden from screen readers and only one skin ever shows it. The list is
// printed twice because a seamless loop needs a second copy to scroll into.
function ticker() {
  const played = state.fixtures.filter(L.isPlayed)
    .sort((a, b) => (L.whenPlayed(a) < L.whenPlayed(b) ? 1 : -1)).slice(0, 10);
  const next = L.nextFixture(state.fixtures, state.meta.team, todayIso());
  const bits = played.map((f) => `<span class="it${f.home === state.meta.team || f.away === state.meta.team ? ' us' : ''}">
    ${esc(f.home)} <b>${f.hs}–${f.as}</b> ${esc(f.away)}</span>`);
  if (next) bits.unshift(`<span class="it nx">Next · ${esc(next.home === state.meta.team ? next.away : next.home)}
    · ${esc(shortDate(L.whenPlayed(next)))} ${esc(next.time)}</span>`);
...
function render() {
  ...
  $('#app').innerHTML = `<div class="wrap">
    <header class="mast">
      ...
      <div class="skin" role="group" aria-label="Page style">
        ${SKINS.map(([id, label]) => `<button type="button" data-skin="${id}"
          aria-pressed="${skin() === id}">${label}</button>`).join('')}
      </div>
    </header>
    ${ticker()}
    ${hero(next)}
...
function dataFlags() {
  const { offBy, unknown } = L.dataChecks(state.fixtures, state.appearances, state.squad, state.meta.team, state.formations, state.motm);
...
document.addEventListener('click', (e) => {
  const sk = e.target.closest?.('.skin button');
  if (sk) { setSkin(sk.dataset.skin); return; }
