...
function lineup(f) {
  ...
  const chips = lines.map((line) => line.players.map((sp, i) => {
    ...
    return `<div class="chip" style="left:${x.toFixed(1)}%;top:${line.y}%">
      ...
      <span class="nm">${esc(name(sp.player))}</span></div>`;
  }).join('')).join('');
  ...
  return `<section class="full"><h2>Line-up</h2>
    ...
    <div class="pitchwrap">
      <div class="pitch"><div class="box"></div><div class="box six"></div><div class="box top"></div><div class="box six top"></div>${chips}</div>
      ${other.length ? `<div class="strip"><span class="lbl">Also</span>
        ${other.map((sp) => `<span class="sub">${face(sp.player)}<b>${esc(sp.pos)}</b>${esc(name(sp.player))}</span>`).join('')}</div>` : ''}
      ${bench.length ? `<div class="strip"><span class="lbl">Bench</span>
        ${bench.map((sp) => `<span class="sub">${face(sp.player)}${esc(name(sp.player))}</span>`).join('')}</div>` : ''}
    </div></section>`;
}
