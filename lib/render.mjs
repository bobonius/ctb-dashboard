...
function attendance() {
  ...
  const cell = (p, f) => {
    ...
    if (!logged.has(f.id)) return `<span class="cell nl" title="${esc(when)} — not logged"><i class="dot"></i></span>`;
    const a = ap.find((x) => x.player === p.id && x.fixture === f.id);
    if (!a) return `<span class="cell off" title="${esc(when)} — missed"><i class="dot"></i></span>`;
    const bits = [];
    ...
    let inner;
    if (a.goals) {
      inner = icon('i-ball', 'ball')
        + (a.goals > 1 ? `<i class="n">${a.goals}</i>` : '')
        + (a.assists ? icon('i-boot', 'boot tuck') : '');
    } else if (a.assists) {
      inner = icon('i-boot', 'boot') + (a.assists > 1 ? `<i class="n a">${a.assists}</i>` : '');
    } else {
      inner = '<i class="dot"></i>';
    }
    return `<span class="cell" title="${esc(tip)}">${inner}</span>`;
  };
