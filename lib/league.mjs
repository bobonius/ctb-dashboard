...
// A semicolon separates as well as a comma, and a field full of semicolons
// contains no comma, so it needs no quotes. That is the difference between
// logging a match from a phone and fighting a keyboard that curls every quote.
// Both still work, so rows written the old way keep parsing.
...
export function dataChecks(fixtures, appearances, squad, team, formations = new Map(), motm = new Map()) {
