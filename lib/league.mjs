// The only copy of the league logic. lib/render.mjs imports it and does no
// arithmetic of its own: if a number on the page is wrong, it is wrong here,
// and fixing it here fixes every place it appears. Do not reimplement any of
// this anywhere else.
...
// The starting eleven for a match, written before it is played. It lives in
// the same row as the result but in its own column, because `squad` means
// "who played" — putting a plan there would hand everyone an appearance for a
// match that has not kicked off.
//
//   14,,,,GK:Bob;CB:Tom;CB:Marlo;CM:Loek;ST:Jesse;BENCH:Stefan
//
// Labels repeat freely; order within a line is left to right as written.
export function readFormations(csvText) {
  const out = new Map();
  parseDelimited(csvText).forEach((r) => {
    const spots = slugs(r.formation).map((x) => {
      const i = x.indexOf(':');
      return i < 0
        ? { pos: '', player: x }
        : { pos: x.slice(0, i).trim().toUpperCase(), player: x.slice(i + 1).trim() };
    }).filter((x) => x.player);
    if (spots.length) out.set(String(r.fixture_id), spots);
  });
  return out;
}

// Where each label sits on a pitch drawn with our goal at the bottom.
export const LINES = [
  { key: 'gk', y: 88, labels: ['GK', 'K'] },
  { key: 'def', y: 70, labels: ['LB', 'LWB', 'CB', 'RB', 'RWB', 'SW', 'DF', 'D'] },
  { key: 'dm', y: 57, labels: ['CDM', 'DM'] },
  { key: 'mid', y: 44, labels: ['LM', 'CM', 'RM', 'LCM', 'RCM', 'MF', 'M'] },
  { key: 'am', y: 31, labels: ['CAM', 'AM'] },
  { key: 'att', y: 19, labels: ['LW', 'RW', 'ST', 'CF', 'SS', 'FW', 'F'] },
];

export const BENCH = 'BENCH';

// Splits a formation into pitch lines, bench and anything with a label we do
// not recognise — which is shown rather than dropped, because the positions
// are fluid and inventing one should cost nothing.
export function layout(spots) {
  const lines = LINES.map((l) => ({ ...l, players: [] }));
  const bench = [];
  const other = [];
  (spots || []).forEach((s) => {
    if (s.pos === BENCH || s.pos === 'SUB') { bench.push(s); return; }
    const line = lines.find((l) => l.labels.includes(s.pos));
    if (line) line.players.push(s);
    else other.push(s);
  });
  // A label that names a side should get that side. Everything else keeps the
  // order it was written in.
  const side = (p) => (p.pos.startsWith('L') ? 0 : p.pos.startsWith('R') ? 2 : 1);
  lines.forEach((l) => l.players.sort((a, b) => side(a) - side(b)));
  return { lines: lines.filter((l) => l.players.length), bench, other };
}
...
export function readSquad(csvText) {
  return parseDelimited(csvText).map((r) => ({
    ...
    // a file in data/faces/, or a full path. Empty means nobody has asked yet;
    // "no" means they were asked and declined. Both render a silhouette — the
    // difference is only so you can tell one from the other in the file.
    photo: ['no', 'none', '-'].includes(String(r.photo || '').trim().toLowerCase()) ? '' : (r.photo || ''),
  }));
}

// Man of the match: one id, in its own column on the same row. It is an opinion
// rather than a fact, so it never feeds the totals — it is only ever displayed.
export function readMotm(csvText) {
  const out = new Map();
  parseDelimited(csvText).forEach((r) => {
    const id = slugs(r.motm)[0];
    if (id) out.set(String(r.fixture_id), id);
  });
  return out;
}

// A list of ids inside one cell. A semicolon separates as well as a comma, and
// a field full of semicolons contains no comma, so it needs no quotes — the
// difference between logging a match from a phone and fighting a keyboard that
// curls every quote. Phone keyboards set to a CJK layout also slip in the
// full-width ； ： ，, which look identical and are not the same characters;
// they are folded to their ASCII twins first, so "Chris:1；Marlo：1" is two
// scorers and not one scorer called "Chris" with a nonsense count.
const FULLWIDTH = { '\uFF1B': ';', '\uFF1A': ':', '\uFF0C': ',' };
const slugs = (v) => String(v ?? '')
  .replace(/[\uFF1B\uFF1A\uFF0C]/g, (c) => FULLWIDTH[c])
...
//
// `squad` is who played. `goals` and `assists` are ids, `:n` for more than
...
// The fixture id of every row, in file order, exactly as typed. The expanded
// records above cannot show a row that was logged twice or that points at no
// match, because by then the damage is already folded into the counts.
export const appearanceRowIds = (csvText) =>
  parseDelimited(csvText).map((r) => String(r.fixture_id ?? '').trim());
...
export function leaderboard(squad, appearances, key) {
  return squad
    .map((p) => ({ id: p.id, name: p.name, ...playerTotals(appearances, p.id) }))
    .filter((r) => r[key] > 0)
    .sort((a, b) => b[key] - a[key] || a.apps - b.apps || a.name.localeCompare(b.name));
}

// Everything the hand-typed data can contradict, all of which has happened:
//   offBy     scorers that do not add up to the score
//   unknown   ids that are in no squad row
//   orphans   rows whose fixture_id is no fixture — a typo, or a merge that left
//             its <<<<<<< ======= >>>>>>> markers in the file
//   twice     a match logged on two rows, which counts everyone in it twice
//   notOurs   a row pointing at a real fixture that we did not play in — what a
//             mistyped id usually lands on, since most ids belong to other teams
// None of these break the page, which is exactly why they need saying out loud.
export function dataChecks(fixtures, appearances, squad, team, formations = new Map(), motm = new Map(), rowIds = []) {
  ...
  const offBy = fixtures
    ...
    .filter((x) => x && !x.ok);
  const named = new Set(appearances.map((a) => a.player));
  formations.forEach((spots) => spots.forEach((x) => named.add(x.player)));
  motm.forEach((id) => named.add(id));
  const unknown = [...named].filter((p) => !known.has(p)).sort();

  const fixtureIds = new Set(fixtures.map((f) => f.id));
  const orphans = [...new Set(rowIds.filter((id) => !fixtureIds.has(id)))];
  const conflict = orphans.some((id) => /^(<{7}|={7}|>{7})/.test(id));
  const counts = new Map();
  rowIds.filter((id) => fixtureIds.has(id)).forEach((id) => counts.set(id, (counts.get(id) || 0) + 1));
  const twice = [...counts].filter(([, n]) => n > 1).map(([id]) => fixtures.find((f) => f.id === id));
  const notOurs = [...counts.keys()].map((id) => fixtures.find((f) => f.id === id))
    .filter((f) => f.home !== team && f.away !== team);

  return { offBy, unknown, orphans, conflict, twice, notOurs };
}
