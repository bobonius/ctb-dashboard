...
import { parseDelimited, int } from './csv.mjs';
...
export function readPositions(csvText) {
  return parseDelimited(csvText).map((r) => ({
    date: r.date,
    pos: int(r.position),
    pts: int(r.points) ?? 0,
...
// A date is complete when every fixture scheduled on or before it has a score.
// Postponing a match means moving its scheduled date forward, which removes it
// from the earlier week's expectation rather than blocking that week forever.
export function isComplete(fixtures, date) {
  return !fixtures.some((f) => f.scheduled <= date && !isPlayed(f));
}

export function missingUpTo(fixtures, date) {
  return fixtures.filter((f) => f.scheduled <= date && !isPlayed(f));
}
...
// Derived points for every complete matchday. scripts/snapshot.mjs writes
// these to history/positions.csv; the browser reads that file rather than
// replaying the season on load.
export function derivePositions(fixtures, team) {
  return matchdays(fixtures)
    .filter((d) => isComplete(fixtures, d))
    .map((d) => {
      const r = rowFor(buildTable(fixtures, d), team);
      return r && { date: d, pos: r.pos, pts: r.pts, source: 'derived' };
    })
    .filter(Boolean);
}

// Hand-entered rows in positions.csv always win over a derived row for the
// same date, so a week you typed off Playpass is never silently overwritten.
export function mergePositions(derived, stored) {
  const manual = stored.filter((p) => p.source === 'manual');
  const byDate = new Map(derived.map((p) => [p.date, p]));
  manual.forEach((p) => byDate.set(p.date, p));
